...
| Entorno | Servidor | Usuario | Ruta | Remote git | Node |
|---------|----------|---------|------|------------|------|
| Local | — | — | `~/SitesLR12/unblock` | HTTPS | pnpm nativo |
| central | central.xerintel.com | `unblock` | `~/unblock` | HTTPS | `npx --yes pnpm` |
| amazzal | amazzal.tabratino.com | `laravel` | `~/domains/unblock.castris.com/unblock/` | SSH (`git@github.com`) | nvm Node 24 (`npx --yes pnpm`) |

### Proceso de actualización

1. Mergear PRs en GitHub (main protegida: requiere PR + check "quality")
2. En cada entorno: `git pull origin main`
3. `composer update <paquetes> -W --no-interaction` (composer.lock está en .gitignore)
4. `npx --yes pnpm install --no-frozen-lockfile && npx pnpm run build`

### Notas de despliegue

- **composer.lock en .gitignore**: cada entorno resuelve dependencias desde composer.json. Usar `composer update <paquetes> -W` para actualizar paquetes específicos.
- **public/.htaccess en .gitignore**: cada servidor mantiene su propia versión con el handler cPanel (ea-php84). No se trackea en git.
- **Branch protection**: `strict: true` + `enforce_admins: true`. No hay bypass ni para admins. Las PRs de dependabot deben mergearse secuencialmente (update-branch → esperar check ~6min → merge → siguiente).
- **amazzal**: tiene Node 12 del sistema (Ubuntu 22.04). Se instaló nvm para user `laravel` con Node 24 como default. Ejecutar siempre como `laravel`, no como root (git SSH keys + nvm).
- **central**: tiene npm del sistema pero no pnpm. Usar `npx --yes pnpm` como wrapper.

---

## 📖 LECCIONES APRENDIDAS

### 2026-02-10 — Despliegue multi-entorno con dependabot y branch protection estricta
**Contexto:** Actualización de 7 PRs de dependabot + despliegue en 3 entornos
**Problema:** Branch protection con `strict: true` impide merge masivo de PRs; composer.lock gitignored causa fallos con `composer install`; amazzal con Node 12 no puede ejecutar pnpm/vite
**Causa raíz:** (1) Strict protection requiere branch actualizada antes de merge → secuencial obligatorio. (2) Sin lock file, `composer install` rechaza versiones nuevas. (3) Ubuntu 22.04 de amazzal trae Node 12 obsoleto.
**Solución:** (1) Ciclo update-branch → wait 6min → merge por cada PR. (2) `composer update <paquetes> -W` en lugar de install. (3) nvm + Node 24 para user `laravel` en amazzal.
**Regla derivada:** Documentar proceso de despliegue en CLAUDE.md. Siempre usar `composer update -W` en entornos sin lock file. Verificar versión de Node antes de primer build en servidor nuevo.
